...
使い方:
  ...
  bin/kb_build.py --limit 3                # 生成する技術ノートを先頭 N 件に制限（動作確認用。seen は空で保存）
  bin/kb_build.py --dry-run                # claude を呼ばずプロンプト/インデックスのみ出力
  bin/kb_build.py --taxonomy-only          # タクソノミー(JSON)生成まで（ノートは作らない。seen は空で保存）

--limit / --taxonomy-only ではノートに未取り込みの digest が残るため、状態(_taxonomy.json)の
seen は空で保存する。次回の通常実行が増分更新として全 digest をノートへ取り込む。
"""
import glob
import os
import re
...
NOTE_TIMEOUT = 600
NOTE_CONCURRENCY = 3                # ノート生成の同時 claude 実行数
...
2. frontmatter の tags に最低限: `knowledge-base` / `tech/{slug}` / 寄与プロジェクトごとに `project/<id>`。
   ...
   frontmatter の文字列値（title 等）はダブルクオート（`"..."`）で囲む。
...
def build_note_prompt(tech, source_digests, template):
    ...
    for i, d in enumerate(source_digests):
        ...
        if total + len(block) > MAX_NOTE_SOURCE_CHARS:
            ...
            omitted = [x["id"] for x in source_digests[i:]]
            break
        blocks.append(block)
        total += len(block)
...
def normalize_taxonomy(raw, valid_ids):
    ...
    # related は実在する slug のみ残す（[[リンク]] 切れ防止）
    valid_slugs = set(t["slug"] for t in techs)
    for t in techs:
        t["related"] = [r for r in t["related"] if r in valid_slugs]
    return techs
...
def build_merge_prompt(tech, existing_note, new_digests, updated):
    ...
    for i, d in enumerate(new_digests):
        ...
        if total + len(block) > MAX_NOTE_SOURCE_CHARS:
            blocks.append("…(入力が長いため以降の digest を省略)")
            omitted = [x["id"] for x in new_digests[i:]]
            break
        blocks.append(block)
        total += len(block)
...
def run_incremental(home, note_subdir, digests, state, opts, updated):
    ...
    for nt in (raw.get("new_technologies") or []):
        ...
        bymap[slug] = t
    # related は実在する slug のみ残す（[[リンク]] 切れ防止）
    valid_slugs = set(bymap.keys())
    for t in techs:
        t["related"] = [r for r in (t.get("related") or []) if r in valid_slugs]
    ...
    # 3.5) 割当ゼロのまま残った新領域（sources 空）は index/状態に載せない
    techs = [t for t in techs if t["sources"]]

    # 4) 影響を受けた領域のみノート更新（プロンプトを先に組み立て、同時 NOTE_CONCURRENCY 件で並列生成。
    #    stderr が混ざらないよう、各ノートの結果表示は全件完了後にまとめて出す）
    template = K.load_template("tech_note")
    tasks = []  # (slug, mode, out_path, prompt, omitted)
    for slug in sorted(added_by_slug):
        ...
        if os.path.exists(out_path):
            ...
            prompt, omitted = build_merge_prompt(t, existing_note, added_srcs, updated)
            mode = "更新"
        else:
            # ノートが無い（新領域 or 消失）→ その領域の全 sources から新規生成
            all_srcs = [by_id[i] for i in t["sources"] if i in by_id]
            prompt, omitted = build_note_prompt(t, all_srcs or added_srcs, template)
            mode = "新規"
        tasks.append((slug, mode, out_path, prompt, omitted))
    sys.stderr.write("[kb] 増分更新: %d 領域のノートを並列生成中（同時 %d）...\n"
                     % (len(tasks), NOTE_CONCURRENCY))
    results = K.run_claude_many([task[3] for task in tasks],
                                timeout=NOTE_TIMEOUT, concurrency=NOTE_CONCURRENCY)

    updated_cnt, created_cnt, failed = 0, 0, 0
    unseen = set()  # 取り込めなかった digest（失敗領域に紐づく新規分 / 入力上限で省略した分）
    for (slug, mode, out_path, prompt, omitted), (ok, result) in zip(tasks, results):
        t = bymap[slug]
        if omitted:
            sys.stderr.write("[kb] 警告(%s): 入力上限超過のため digest を省略: %s\n"
                             % (slug, ", ".join(omitted)))
            t["sources"] = [s for s in t["sources"] if s not in set(omitted)]
            unseen |= set(omitted)
        if _write_note_guarded(out_path, ok, result):
            if mode == "更新":
                ...
                created_cnt += 1
            sys.stderr.write("[kb] 増分%s: %s (+%d digest) 完了\n"
                             % (mode, slug, len(added_by_slug[slug])))
        else:
            ...
            failed += 1
            # 失敗領域に紐づく新規 digest は seen に入れず、次回実行で自動リトライさせる
            unseen |= set(added_by_slug[slug])

    # 5) seen を更新（失敗/省略分は除外して次回リトライ。未割当の新規 digest は seen に入れて
    #    二度と分類し直さない）し、状態・index を保存
    seen |= set(new_ids) - unseen
    save_state(home, techs, seen, updated)
    idx = write_index(home, note_subdir, techs, updated)
...
def main():
    ...
    if opts["from_taxonomy"]:
        ...
        rc, unseen = run_pass_b(home, note_subdir, digests, techs, opts, updated)
        if opts["limit"]:
            # --limit は動作確認用: 未生成ノートが残るので seen は空で保存し、次回に全件を取り込ませる
            save_state(home, techs, set(), updated)
        else:
            save_state(home, techs, valid_ids - unseen, updated)  # 以降は増分更新に乗せる
        return rc
    ...
    if opts["taxonomy_only"]:
        # ノートは未生成なので seen は空で保存する（取り込み済み扱いにせず、
        # 次回の通常実行で全 digest がノートへ取り込まれるようにする）
        save_state(home, techs, set(), updated)
        idx = write_index(home, note_subdir, techs, updated)
    ...
    # --limit 時は未生成ノートが残るので seen は空で保存し、次回に全件を取り込ませる。
    # 失敗/省略分（unseen）は seen に入れず、次回実行で自動リトライさせる。
    if opts["limit"]:
        save_state(home, techs, set(), updated)
    else:
        save_state(home, techs, valid_ids - unseen, updated)
    return rc


def run_pass_b(home, note_subdir, digests, techs, opts, updated):
    """パスB: 技術領域ごとに Obsidian ノートを生成し、最後に index.md を書く。
    (リターンコード, seen に入れない digest id 集合) を返す。"""
    notes_dir = os.path.join(home, note_subdir)
    os.makedirs(notes_dir, exist_ok=True)
    template = K.load_template("tech_note")
    by_id = {d["id"]: d for d in digests}
    ...
    # プロンプトを先に組み立て、同時 NOTE_CONCURRENCY 件で並列生成する。
    # stderr が混ざらないよう、各ノートの結果表示は全件完了後にまとめて出す。
    tasks = []  # (tech, out_path, prompt, omitted)
    for t in target_techs:
        srcs = [by_id[i] for i in t["sources"] if i in by_id]
        if not srcs:
            continue
        prompt, omitted = build_note_prompt(t, srcs, template)
        tasks.append((t, os.path.join(notes_dir, "%s.md" % t["slug"]), prompt, omitted))
    sys.stderr.write("[kb] パスB: %d 領域のノートを並列生成中（同時 %d）...\n"
                     % (len(tasks), NOTE_CONCURRENCY))
    results = K.run_claude_many([task[2] for task in tasks],
                                timeout=NOTE_TIMEOUT, concurrency=NOTE_CONCURRENCY)

    generated, failed = 0, 0
    unseen = set()  # 取り込めなかった digest（失敗領域の sources / 入力上限で省略した分）
    for (t, out_path, prompt, omitted), (ok, result) in zip(tasks, results):
        if omitted:
            sys.stderr.write("[kb] 警告(%s): 入力上限超過のため digest を省略: %s\n"
                             % (t["slug"], ", ".join(omitted)))
            t["sources"] = [s for s in t["sources"] if s not in set(omitted)]
        ...
        if _write_note_guarded(out_path, ok, result):
            generated += 1
            sys.stderr.write("[kb] パスB: %s (%d sources) 完了\n" % (t["slug"], len(t["sources"])))
        else:
            with open(out_path + ".prompt.txt", "w", encoding="utf-8") as f:
                f.write(prompt)
            sys.stderr.write("[kb] パスB 失敗(%s): %s -> プロンプトを保存\n"
                             % (t["slug"], result if not ok else "非ノート出力"))
            failed += 1
            # 失敗領域の digest は seen に入れず、次回実行で自動リトライさせる
            unseen |= set(t["sources"])
