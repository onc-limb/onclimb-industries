...
description: >-
  音声文字起こし(Plaud / Teams 等)から、社内共有できる議事録を 1 本生成する。対話で参加者・会議種別・公開範囲・目的を確定してから、目的 / 会話の流れ / 全体要約 / 決定事項 / 保留 / TODO を抽出する。会議録音のほか、会議体でない口頭のやり取りで何かが決まった録音・AI エージェントとの電話・同僚との雑談も同じ流儀で扱い、話者が匿名やベタ書きでも対話で確定してから生成する。「議事録にして」「決定事項と TODO を抜いてまとめて」「この transcript を議事録化して」等、文字起こしファイルを指した明示依頼時のみ起動(自動起動しない)。
model: sonnet
effort: medium
metadata:
  type: skill
...
1. **会議種別**: 会議 / 口頭決定 / AI エージェントとの電話 / 雑談（出力テンプレを切り替える）。
...
2. 会議種別に応じたテンプレで組み立てる:
   ...
   - `informal_decision` → [`templates/informal_decision.md`](templates/informal_decision.md)
   - `agent_call` → [`templates/agent_call.md`](templates/agent_call.md)
   - `chat` → [`templates/chat.md`](templates/chat.md)
   - 地の文（目的・全体要約・会話の流れの要約部分）にだけ、[`personas/writing-style.md`](../../../personas/writing-style.md)
...
7. **ToDo 台帳への収穫**（jarvis-todo-management との連携）: 議事録の TODO のうち
