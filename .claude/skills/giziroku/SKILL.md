...
- `config/glossary.yaml` `config/roster.yaml` は **プロジェクト横断で共有・蓄積**する資産
  （使うほど誤変換補正と話者推定が当たるようになる）。
...
2. **参加者マッピング**: 話者ごとに「発言量＋抜粋」を提示し、`roster.yaml` の候補＋自由入力で
...
1. **誤変換の正規化**: [`config/glossary.yaml`](config/glossary.yaml) と文脈で固有語・専門語を補正する。
...
- ユーザーが確定した参加者マッピング → `config/roster.yaml` に追記（横断共有、次回の候補に使う）。
- ユーザーが訂正した誤変換 → `config/glossary.yaml` に追記（横断共有、補正精度が上がる）。
- どの確認質問が毎回不要だったかは、運用しながら Phase B の初期推定を強める材料にする。

`roster.yaml` は固有名（機密）を含みうるため、共有ファイル自体は公開対象から外す前提で扱う。
