...
description: >-
  フリーランス本人専用の思考整理スキル。会話・思考・作業ログから「課題発見 → 課題整理 → 解決定義 → 手段検討」の 4 フェーズを判定し、手段先行や目的喪失を批判的に指摘してオープン質問で深掘りする。最終アウトプットは自分用 md (4 章立て・重要/付随で分離) とクライアント向け HTML (Minto 流: 結論 → 根拠 → 提案 → 依頼)。「思考を整理したい」「考えをまとめたい」「一旦落ち着きたい」「目的ってなんだっけ」「一旦まとめよう」など、整理系発話やメタ自己制御発話で起動する。自己進化を備え、判断軸と出力フォーマットは使用ログから磨かれるが、対話スタイル (全肯定禁止 / 選択肢提示禁止 / オープン質問のみ) と 4 フェーズ構造は固定で進化対象外。
model: opus
effort: high
metadata:
  type: skill
