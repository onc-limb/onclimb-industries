...
description: >-
  説明練習の添削(説明練習パイプラインの Step 3)。griot-explain-prep で作った資料を見ながら口頭説明した内容の文字起こしを入力に、どのような相手に説明したか(ペルソナ)を毎回確認したうえで、そのペルソナに伝わるかの観点(結論先出し・構成・聞き手適合・具体性・抜け)で添削する。指摘は固定カテゴリで台帳に蓄積し、定期的な苦手分析に使う。「この説明を添削して」「今日の説明練習の文字起こしを見て」で添削、「苦手分析して」「説明練習の振り返りして」で定期分析を起動。
model: sonnet
effort: medium
metadata:
  type: skill
