...
description: >-
  散在する稼働の手がかり(jarvis-worklog の分類済みログ、Google カレンダー)から、案件別・日別の稼働時間サマリを作る。Claude はソースの読み取りと案件分類(確証がなければ「未分類」に倒す)だけを担い、合算・内訳・重複警告は決定論スクリプトが計算する(合計を暗算しない)。出力は月次エントリ JSON と稼働サマリ Markdown で、ultron-invoice-builder の入力になり得る。「今月の稼働時間を集計して」「案件別稼働サマリを出して」「請求書用に稼働内訳をまとめて」「○○案件に何時間使ったか出して」等の明示依頼時のみ起動(自動起動しない)。勤怠の公式記録や請求書そのものではない。
model: sonnet
effort: low
metadata:
  type: skill
