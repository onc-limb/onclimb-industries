#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""summarize: 分類済みログを入力に claude -p で 3 形式の報告書を生成する。

3 形式:
  progress     … 作業報告書（やったこと/進捗/次の予定/課題）  -> reports/progress/
  deliverables … 成果報告書（成果物と判断根拠・試行錯誤）      -> reports/deliverables/
  knowledge    … ナレッジの種（再利用可能な知見）              -> reports/knowledge/

要約プロンプトの必須要件（本システムの存在理由）:
...
使い方:
  ...
  bin/summarize.py --formats progress 2026-06-22
  bin/summarize.py --dry-run ...           # claude を呼ばずプロンプトだけ生成

claude CLI が見つからない/失敗した場合は、合成プロンプトを reports/<type>/<...>.prompt.txt
...
MAX_LOG_CHARS = 180000  # 1 レポートあたりのログ投入上限（コンテキスト保護）

BASE_INSTRUCTION = """\
あなたは SES エンジニアの作業ログを報告書に整形するアシスタントです。
以下の作業ログ（Claude Code の操作記録を構造抽出・マスキング済み JSONL を読みやすく整形したもの）を要約してください。

# 厳守事項
1. 残ったノイズ（重複したやり取り、冗長なツール出力、空メッセージ、進捗メッセージ、機械的な反復）は要約から除外する。
2. **過程を必ず残す**: 何を試し、なぜその判断をし、何が失敗し、何が効いたか。最終結果だけの要約は不可。これがこの報告書の存在理由。
...
def main():
    ...
    for pid, d, path in targets:
        ...
        for fmt in formats:
            out_dir = os.path.join(home, "reports", fmt)
