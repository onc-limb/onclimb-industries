...
出力(JSON): { "candidates": [ {suggested_id, path_glob, repo, cwds, entries, sessions, dates, sample_bodies}, ... ] }
（同一リポジトリ配下の複数 cwd は path_glob 単位で 1 候補にマージし、cwds に列挙する）
"""
import glob
import json
import os
...
def main():
    ...
    # 同一リポジトリ配下の複数 cwd（サブディレクトリ作業）を path_glob 単位で 1 候補にマージ
    merged = {}  # path_glob -> {cwds, is_git_repo, entries, sessions:set, dates:set, bodies:[]}
    for cwd, g in groups.items():
        top = git_toplevel(cwd)
        path_glob = top or cwd
        m = merged.setdefault(path_glob, {
            "cwds": set(), "is_git_repo": top is not None,
            "entries": 0, "sessions": set(), "dates": set(), "bodies": [],
        })
        m["cwds"].add(cwd)
        m["entries"] += g["entries"]
    ...
    candidates = []
    for path_glob, m in sorted(merged.items(), key=lambda kv: -kv[1]["entries"]):
        cwds = sorted(m["cwds"])
        repo = os.path.basename(path_glob) if m["is_git_repo"] \
            else (W.git_repo_name(cwds[0]) or os.path.basename(cwds[0]))
