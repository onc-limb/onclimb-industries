...
name: jarvis-worklog
description: >-
  Claude Code の会話・ツール操作ログを収集 → 分類 → 整理(プロジェクト視点 / 技術者視点の 2 形式) → アーカイブ退避する作業記録システム。最終成果物だけでなく「何を考え・何を試したか」の過程を残し、後段のスキルが報告書へ整形するための詳細な整理情報を出す(最終報告書そのものではない)。「作業ログ」「今日の作業をまとめて」「6/28 の作業をまとめて」「昨日の分を整理して」「技術整理」等で起動(日付指定に対応。既定は当日)。生ログは削除せず zip 退避するため消失しない。
model: sonnet
effort: medium
metadata:
  type: skill
...
- **整理のみ**: `python3 "$SKILL/bin/summarize.py" [YYYY-MM-DD] [project] [--formats project,tech] [--force] [--dry-run]`
  ...
  - 時間帯分割した digest には、band 間で同じ作業が重複・食い違っていないかを検知した
    `## 【時間帯間の重複・矛盾（下流での突き合わせ用）】` セクションを冒頭に自動生成する
    （並行セッションが別 band に分かれると同一作業が別内容で書かれうるため）。これは「ユーザーに
    確認する候補」であり正誤は判定しない。解消は下流（friday-daily-report 等の確認対話）が行う。
  - ログに無い情報は「記録なし」に加えて可能なら欠損の型を添える（`記録なし（セッション中断）` /
    `記録なし（ユーザー手動・ログ外）` / `記録なし（背景の記載なし）`）。下流
    （friday-daily-report 等の確認対話）がヒアリングの聞き方を変えるために使う。
...
- このメモは、日次報告（friday-daily-report）の対話で「ログに写らない手動作業」を
  確認する際の原料になる。
...
- **原料保全**（personas/jarvis.md）: digest はきれいな要約より情報の保全を優先する。
  ...
  （Problem / トラブルシュート表 / 検証結果は、下流の friday 系（日次報告など）の品質を決める原料）。
