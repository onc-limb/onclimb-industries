...
タスクの種類ごとに使うモデルと effort を `.Codex/agents/` に定義してある。
...
`Codex --agent review` のように起動すると、メインスレッド自体がその model / effort になる。

定義を増やす・しきい値を変えるときは `.Codex/agents/*.md` の frontmatter
（`model`: sonnet/opus/haiku/fable/フルID/inherit、`effort`: low/medium/high/xhigh/max）を編集する。

### スキル側の指定

`.Codex/skills/*/SKILL.md` の frontmatter も `model` と `effort` を持てる（全スキルに設定済み）。
...
### スキル一覧の予算（description が Codex に届くための設定）

Codex に渡されるスキル一覧には文字数予算があり（`skillListingBudgetFraction`、既定は
...
（200k コンテキストで 8,000 字）では収まらないため、`.Codex/settings.json` で 5% に上げてある。
...
スキルは `.Codex/skills/` 配下に置き、**分類プレフィックス + スキル名**（例: `jarvis-worklog`）で命名する。
プレフィックスと分類の対応は [`.Codex/skills/README.md`](.Codex/skills/README.md) を参照。
...
（詳細は `.Codex/skills/README.md` の「friday 系の 2 段構成」。固定パイプライン型の
`friday-giziroku` / `friday-daily-report` は対象外）。

### 新しく Skill を作るとき

1. スキルの分類を決め、対応するプレフィックスを選ぶ（`.Codex/skills/README.md` の一覧）。
...
5. 新しい分類が必要な場合は、先に `personas/<prefix>.md` を作成して共通ルールを定義し、
   `.Codex/skills/README.md` のプレフィックス一覧も更新する。
...
- 操作は必ず `.Codex/skills/jarvis-todo-management/scripts/todo.py` 経由（todos.json を直接編集しない）。
