...
name: jarvis-worklog
description: >-
  Codex の会話・ツール操作ログを収集 → 分類 → 整理(プロジェクト視点 / 技術者視点の 2 形式) → アーカイブ退避する作業記録システム。最終成果物だけでなく「何を考え・何を試したか」の過程を残し、後段のスキルが報告書へ整形するための詳細な整理情報を出す(最終報告書そのものではない)。「作業ログ」「今日の作業をまとめて」「6/28 の作業をまとめて」「昨日の分を整理して」「技術整理」等で起動(日付指定に対応。既定は当日)。生ログは削除せず zip 退避するため消失しない。
model: sonnet
effort: medium
metadata:
  type: skill
...
- ツール本体・設定: このスキルディレクトリ `.Codex/skills/jarvis-worklog/`（`bin/` `config/` `templates/`）
...
```bash
SKILL=/Users/satoshi-onga/Documents/onclimb-industries/.Codex/skills/jarvis-worklog
...
2. 候補があれば **Codex が調査する**: 候補の `cwds`（や `path_glob`）の README / package.json / git remote 等を
...
- **未分類の調査（プロジェクト登録の提案）**: `python3 "$SKILL/bin/suggest_projects.py" [YYYY-MM-DD]`
  ...
  - 出力を受けて Codex が候補を調査・確認し、ユーザー合意のうえ `config/projects.yaml` を編集して再分類する。
    手順は「標準フロー」の『未分類プロジェクトの調査と登録』を参照。
- **整理のみ**: `python3 "$SKILL/bin/summarize.py" [YYYY-MM-DD] [project] [--formats project,tech] [--force] [--dry-run]`
  - `Codex -p`（`--model sonnet`＝現行 Sonnet に追従）をヘッドレス実行し `digests/{project,tech}/` に整理情報を生成。`Codex` が無い/失敗時はプロンプトを `.prompt.txt` に保存し、失敗があると exit 1 で終わる。
  - project / tech の両形式が対象のときは **1 回の Codex 呼び出しで両形式を出力**させ、区切り行で分割して保存する（呼び出し回数半減）。区切りの分割に失敗した場合は生出力を `.raw*.txt` に保存して失敗扱いにする。
...
本番への手動適用・手動検証など、Codex のツール操作として残らない作業は収集の限界で
...
- Web 版（Codex.ai）はローカルにログが無いため収集対象外。
