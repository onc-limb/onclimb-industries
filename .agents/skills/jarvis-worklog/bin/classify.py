...
分類の手がかり優先順位（セッション単位で 1 回判定）:
  ...
  ③ LLM 判定（claude -p / Haiku）           … ①② を外したセッションのみ、まとめて 1 回
...
冪等: 毎回 classified/ を raw/ から再構築するが、**内容が変わらないファイルは書き換えない**
（mtime を保持し、summarize の差分スキップ「digest が classified より新しければ再生成しない」を
機能させるため）。今回の分類結果に現れなくなった古い出力は削除する（_unclassified の .gitkeep は残す）。
...
# LLM 判定の設定
LLM_MODEL = "haiku"           # 分類は軽い仕事なので現行 Haiku で十分（確信度ゲートあり）。クオータ節約
...
def write_if_changed(path, content):
    """既存ファイルと内容が同一なら書き換えない（mtime 保持。summarize の差分スキップの前提）。
    書き換えた/新規作成したら True を返す。"""
    try:
...
def main():
    ...
    if use_llm and not W.claude_available():
        ...
        use_llm = False
    os.makedirs(os.path.join(cls_dir, "_unclassified"), exist_ok=True)
    ...
    counts = {}
    written = set()   # 今回の分類結果として書いた/維持したパス。最後にこれ以外を掃除する
    changed, kept = 0, 0
    for rf in raw_files:
        ...
        # 5) 書き出し（プロジェクト×日付単位で全文を組み立て、内容が変わらなければ触らない）
        buckets = {}  # out_pid -> [jsonl 行, ...]
        for sid, s in sessions.items():
            pid, reason = decided[sid]
            out_pid = "_unclassified" if pid == "未分類" else pid
            lines = buckets.setdefault(out_pid, [])
            for e in s["entries"]:
                e["project_id"] = pid
                lines.append(json.dumps(e, ensure_ascii=False))
            counts[out_pid] = counts.get(out_pid, 0) + len(s["entries"])

        for out_pid, lines in buckets.items():
            path = os.path.join(cls_dir, out_pid, "%s.jsonl" % date)
    ...
    # 6) 今回の分類結果に現れなくなった古い出力を掃除する（再分類・登録変更の名残）
    for old in glob.glob(os.path.join(cls_dir, "*", "%s.jsonl" % (date_filter or "*"))):
        if old not in written:
            os.remove(old)

    summary = ", ".join("%s=%d" % (k, v) for k, v in sorted(counts.items()))
    sys.stderr.write("[classify] %s（書換=%d 変更なし=%d）\n" % (summary or "(0件)", changed, kept))
