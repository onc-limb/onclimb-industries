...
description: >-
  プロジェクト別タスク台帳(todo-data/todos.json)を単一の真実として運用するイベント駆動の ToDo 管理。登録と 30〜60 分単位への分割、作業の区切りでの台帳突き合わせ(該当 ToDo が無ければ自動記載 + 一言通知)、worklog digest・議事録・調査・Google Tasks からの ToDo 収穫、オンデマンド棚卸し(inbox 消化・stale 検知・着手順提案)、Google Tasks への push 同期を担う。事実の記録は自動 + 毎回一言通知、意図の決定(廃棄・優先度・分割確定)は提案してユーザーが確定する。「ToDo 追加」「タスク登録して」「このタスク分割して」「棚卸しして」「これどの ToDo の作業だっけ」「議事録から ToDo 拾って」等で起動。優先度の評価は jarvis-todo-prioritizer の領分。
model: sonnet
effort: low
metadata:
  type: skill
...
| **突き合わせ** | 作業の区切り (AGENTS.md ルールから自動) /「これどの ToDo の作業だっけ」 | フロー C |
...
作業の区切りで呼ばれる中核フロー。ルート AGENTS.md のルールからも起動される。
