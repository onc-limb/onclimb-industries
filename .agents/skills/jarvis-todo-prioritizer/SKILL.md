...
description: >-
  ToDo 台帳のタスクに、影響度(impact 1-5)と緊急度(urgency 1-5)を根拠付きで評価・記録する。材料は 3 系統 — ①プロジェクトの実態(台帳の due/滞留・成果物の現状・worklog digest)、②外部要因(期限・EOL・セキュリティ・制度変更。絡むタスクのみ Web 調査し出典 URL と取得日を付ける)、③ユーザーとの壁打ち(2〜3 件ずつ仮評価を提示して現場感で上書き) — で、確定した評価だけを todo.py で台帳に記録する(スコア計算と並べ替えはスクリプトが行い暗算しない)。「ToDo の優先順位つけて」「どれからやるべき？」「優先度を見直して」等の明示依頼時のみ起動(自動起動しない)。台帳の登録・分割・状態管理・同期は jarvis-todo-management の領分。
model: sonnet
effort: low
metadata:
  type: skill
...
- 操作は必ず jarvis-todo-management の `scripts/todo.py` 経由（直接編集しない）:
  ```bash
  TODO=.Codex/skills/jarvis-todo-management/scripts/todo.py
  ...
  ```
- **スコア計算（impact × urgency）と並べ替えは todo.py が行う**。Codex は暗算しない。
...
3. **ユーザーとの壁打ち**（確定はここ）
   ...
   - Codex の案と違う値で確定した場合も、ユーザーの理由を rationale に残す。
...
5. **結果の提示**: `list --sort priority` の出力を転記し、rubric の象限の目安で
   着手順の提案を添える（決定はユーザー）。
6. 区切りとして AGENTS.md の ToDo 突き合わせ（本セッション自体の作業記録）を行う。
