...
- LLM ジャッジは `Codex` CLI（`Codex -p --output-format json`）で行う。モデルは
  config.json の `judge.command` で指定（既定: `Codex-sonnet-5`）。
...
3. **採点（judge）**: `evalkit.py judge <app> --run latest` — ケースごとにジャッジプロンプトを
   組み立て Codex CLI で採点する。
   - Codex CLI が使えない環境では `--export-prompts` でプロンプトをファイル出力し、
     セッション内の Codex（Agent の並列サブエージェント）で判定して `judgments.jsonl` に
